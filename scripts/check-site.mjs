...
for (const filePath of htmlFiles) {
  ...
  for (const match of html.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g)) {
    ...
    catch { problems.push(`${relativeFile} contains invalid JSON-LD.`); }
  }
  for (const match of html.matchAll(/href="(\/[^"?#]*)/g)) {
    if (!(await internalPathExists(match[1]))) problems.push(`${relativeFile} contains a broken internal link to ${match[1]}.`);
  }
  for (const match of html.matchAll(/<img\b[^>]*>/gi)) {
    if (!/\salt=("[^"]*"|'[^']*')/i.test(match[0])) problems.push(`${relativeFile} contains an image without alt text.`);
  }
  for (const match of html.matchAll(/<iframe\b[^>]*>/gi)) {
    if (!/\stitle=("[^"]+"|'[^']+')/i.test(match[0])) problems.push(`${relativeFile} contains an iframe without an accessible title.`);
  }
...
try {
  ...
  const sig = previewImage.subarray(0, 8).toString("hex");
  const width = previewImage.readUInt32BE(16);
  const height = previewImage.readUInt32BE(20);
  if (sig !== "89504e470d0a1a0a" || width !== 1200 || height !== 630) problems.push(`social-preview.png is ${width}×${height}; expected a 1200×630 PNG.`);
} catch { problems.push("social-preview.png is missing or invalid."); }
...
if (guideCardCount !== 26) problems.push(`The guide index has ${guideCardCount} cards; expected 26.`);

for (const [slug, titleNeedle] of [
  ["amateur-radio-slang-73-88-qth-qso-qsl", "What Does 73 Mean? Amateur Radio Slang Explained"],
  ["amateur-radio-callsigns-explained", "Amateur Radio Callsigns Explained"],
  ["digital-radio-hotspots-pistar-wpsd", "Hotspots Explained"]
]) {
  if (!guidesIndex.includes(`/guides/${slug}`)) problems.push(`The Guides page is missing the ${slug} card.`);
  try {
    const html = await readFile(path.join(publicRoot, `guides-${slug}.html`), "utf8");
    if (!html.includes(titleNeedle)) problems.push(`${slug} is missing its expected SEO title/content.`);
...
try {
  ...
  if (searchIndex.length !== sitemapUrlCount) problems.push(`The full-content search index has ${searchIndex.length} pages; expected ${sitemapUrlCount}.`);
  for (const page of searchIndex) if (!page.title || !page.url || !page.content || page.content.length < 40) problems.push(`The search entry for ${page.url || "an unknown page"} is incomplete.`);
} catch { problems.push("search-index.json is missing or invalid."); }
...
const networkGuide = await readFile(path.join(publicRoot, "guides-internet-linked-radio-networks.html"), "utf8");
if (!networkGuide.includes("International Radio Network") || !networkGuide.includes("ANZEL Radio") || !networkGuide.includes("ZMR Radio") || !networkGuide.includes("Network Radios")) problems.push("The internet-linked radio networks guide is incomplete.");
if (!networkGuide.includes("/guides/qso-one-guide")) problems.push("The network radio guide is missing the QSO One guide link.");

const responsiveCss = await readFile(path.join(publicRoot, "style.css"), "utf8");
if (!responsiveCss.includes("body.menu-open") || !responsiveCss.includes("overflow-wrap: anywhere") || !responsiveCss.includes("max-height: calc(100svh - 68px)")) problems.push("The shared mobile overflow and navigation safeguards are incomplete.");
